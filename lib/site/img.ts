...
// **無料枠は5,000ユニーク変換/月**(ユニーク=元URL×パラメータ組、毎月1日リセット)。
// 2026-09-25に75%到達の通知が来た(実測3,782/5,000)ので、幅の種類を整理した。
// **幅を1種類足すと、その幅で表示される画像の枚数ぶん丸ごと枠を食う**
// (R2のメディアは435点、エピソードアートは約330点)。新しい幅を足す前に、
// 既存の幅で足りないかを必ず考える——表示サイズより大きい幅を共用する方が、
// 専用の小さい幅を作るより枠には優しい(転送量は増えるが、そちらは無料)。
...
// 用途別の幅(表示幅×2=Retina想定)。**種類は増やさない。**
// 2026-09-25に4種類へ減らした。消したものと理由:
//   ep: 320     … エピソード一覧の小サムネ。同じアートをtile(480)でも出していて
//                  二重に数えられていた → tileに統合
//   pick: 240   … studioのプール選択グリッド(管理画面専用)
//   studio: 96  … studioの行内サムネ(管理画面専用)
//     この2つは公開側のどこでも使っていない=**編集作業が公開枠を食っていた**。
//     435点のプールを一度眺めるだけで最大870ユニーク。tileに寄せれば、公開側が
//     既に変換済みの1枚を使い回すので追加はゼロになる(そのぶん転送は増えるが無料)
//   product: 640 … 定義だけ残っていて呼び出しゼロ(2026-09-25時点)
export const IMG_W = {
  tile: 480, // 棚のタイル・エピソード一覧・studioの一覧(カバー・サムネ、表示34〜220px)
