...
export default async function Home() {
  ...
  const [todayRes, updatesRaw, photo, covers, daysRes, artRes, guestRows, pool] = await Promise.all([
    ...
    // **DESKはHomeのUPDATEに出さない(2026-09-22 Andy承認、第3引数true)。**
    // 毎日1本のDESKが5枠を必ず埋め切るので、同じ週に配信した番組の回が
    // 1本も載らない状態になっていた(実測: 9/17〜9/21の5行が全部DESK、
    // その週の4本のエピソードは全滅)。同日に並んだときもDESKが先に積まれるため、
    // 構造として回は永久に出てこない=「UPDATE」という名前が嘘になっていた。
    // DESKはすぐ下の窓と NOTE — LATEST が担うので、Homeでは重複でもある。
    // 経緯: 2026-07-20に「毎日のscribeで埋まりすぎる」として当日分だけにする →
    // 2026-09-11に確定アーカイブを戻す → 同じ問題が再発したのでまた外す。
    // /updates(全量の記録)は変えない。あちらはDESKも含めて流れる場所。
    // 当日のDESK(LIVE行)も出さない(2026-09-11 Andy指定)。1件多く取ってLIVE行を除き5件に切る
    recentUpdates(6, true, true),
