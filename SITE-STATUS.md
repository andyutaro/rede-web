...
- **事故記録: 注釈ポップアップの位置計算でSSRが落ちていた（2026-07-28、同日修正）**: `window.innerWidth`をクライアントコンポーネントの**本体**で読んでいたため、サーバー描画のたびに`ReferenceError: window is not defined`が出て、Reactが`Switched to client rendering because the server rendering errored`でフォールバックしていた。**画面には出るので気づきにくい**（devオーバーレイの「1 Issue」でAndyが発見）。修正: 位置計算を関数に包み、呼ぶのはポップアップを開いた後＝クライアントだけを通る経路に限定。**教訓: 'use client'でもSSRは走る。`window`/`document`はuseEffect・イベントハンドラ・条件付きJSXの中だけで触る**（コンポーネント本体のトップレベルはサーバーでも実行される）。
- **単独URLの再生カード化を表示側でも行う（2026-07-28）**: 放送卓のペースト処理がカード化を担っているが、**スマホの入力経路では貼り付けがpasteイベントを通らないことがあり、素のリンクのまま本文に入る**（Andy報告: スマホからSpotifyのリンクを貼ると再生ボックスにならない）。入力経路に依存しない直し方として、表示側で「そのブロックの中身が単独URLだけ」の場合に再生カードへ昇格させる（`lib/site/upgradeEmbeds.ts`、ScribeArchiveから呼ぶ）。表示側サニタイザが既に素URLをリンク化しているのと同じ層の仕事で、**SSOTには触れない＝過去のアーカイブも遡って直る**。文中のURLは触らない。注釈の適用より先に走らせる（注釈がiframeを包まないように）。
- **観察メモ（未対応）**: サカナカイギの水の上の所属行「Andyのオリジナル番組」はフェード帯に重なりほぼ読めない（ヒーロー実装時から）。直すなら銘板と同じく水の外へ出すか、マスクのフェード開始位置をpx指定に変える。Andyの判断待ち。
- **注釈（2026-07-28 Andy指定、実装済み・要Supabase SQL実行）**: 本文の一部をドラッグして、後から欄外注釈を足す。**本文（SSOT）には一切触れない別レイヤー**＝記法もタグも増やさない（Markdown廃止の原則を裏口から破らない。消せば跡形もなく消える）。アンカーは**本文全体の先頭からの文字オフセット＋引用文字列**で、本文を後から編集してもズレを引用文字列で再探索して吸収する（W3C Web Annotationのtext quote selectorと同じ考え方）。**当初はブロック単位に閉じていたが「ここまでが一段落」という人の直感とDOMのブロック境界が一致せず不自由だったため、同日中にroot基準へ改めた（Andy指摘）**＝長さも段落数も問わずドラッグできる（包む処理はテキストノード単位なので境界を跨いでもspanが複数に分かれるだけで構造は壊れない）。**同じ箇所への二重引きは断る**（既存注釈に掛かる選択は`intersectsNode`で検出）。引かれた箇所は**青＋下線**（`--anno`、「彩色はLIVE赤のみ」の2つ目の例外。赤=いま生きているものと用途が混ざらない色）。表示は**引用範囲に細い下線→タップで小さく開く**（Andy案。余白注釈より優れる: 本文のレイアウトを動かさない・PCとスマホで同じ形・「同じ頃の作品」と下部で競合しない）。**公開サイトから直接編集できる**（Andy要望）: ログイン中は`andyutaro.com`を普通に見ている画面でドラッグ→「注釈」ボタン→保存。書き込み可否は必ずAPI側で再検証（画面側の`canEdit`は表示の都合にすぎない）。匿名訪問者には`isEditor()`がCookie有無だけ見て即false＝Supabaseに問い合わせない（公開ページを1msも重くしない）。DOM操作はテキストノードの一部分だけを`<span>`で包むのでリンクや`<br>`の構造を壊さない（実DOMで検証済み）。対象はscribe個別とNotes/Photography/Physical個別。**要Supabase SQL実行: `db/2026-07-28-annotations.sql`**（未実行でも注釈が出ないだけで本文は正常）。
- **「同じ頃」＝日付を鍵にした全ページ相互リンク（2026-07-28 Andy指定、実装済み）**: 「日付をカギとして、すべてのページを結びつけたい」。scribe個別・エピソード・Notes/Photography/Physical個別の各ページ下部（Pagerの上）に、その日付の**前後7日**に生まれた他の仕事を最大6件出す。**厳密な同日にしない**理由: scribeは毎日・Physicalは年数回と頻度が違いすぎ、同日一致だとPhysical/Photographyでは一生表示されない死んだ機能になる。幅を持たせ各行に実日付を出す＝嘘にならない。**同じ棚は出さない**（前後の移動はPagerの担当。`excludePrefix`で`/scribe/`や`/podcast/<slug>/`を除外＝1つの文字列で全ページ分の規則を表す）。表示は**Photography一覧と同じ正方形の画像グリッド**（`.grid4`＋3段ラベル。2026-07-28 Andy指定「画像の一覧で。タイトルは見切れて構わない」）＝棚と同じ視覚言語のまま。サムネイルは各棚の正典と同じ優先順位（manual＞本文の最初の画像＞保存済み＞充当）で索引に含め、ポッドキャストは回のアート→番組カバーの順。旧「この頃のscribe」（2026-07-20、ORIGINAL限定・scribeのみ）はこれに一般化して廃止＝特例が減った。性能: 日付索引を**エッジキャッシュ**（`dated-index-1`、30分）に持ち、force-dynamicページからもRSS5本＋DB走査を毎回踏まない（photoPoolと同じ手＝1102対策の文法）。中身が無い日はブロックごと出さない。
- **迷惑メール・罵倒の自動隔離（2026-07-27、実装済み・要Supabase SQL実行）**: 「そういうメールは来たことはないが今後のために」（Andy）。`lib/site/spamFilter.ts`のルールベース判定（AI不使用＝費用ゼロ・理由が説明できる）で`/api/contact`受信時にスコアリングし、**隔離分はメール通知を送らない**＝目に入らない。**消さずに隔離**（studioのMAIL室にSPAMタブ、「スパムではない」で受信箱へ復帰。逆に手動で「スパムへ」も可）。隔離理由は本文展開時に表示＝判定を監査できる。材料: リンク本数・営業/勧誘の語（日英）・日本語なし＋リンク・キリル文字・名前欄がURL・本文がほぼリンクのみ・同一本文の連投・**中身のない罵倒のみ**（短文＋強い攻撃語に限定。「つまらない」「期待外れ」等の否定的な感想や、激しい語を含む長文の訴えは通す＝批評は届く）。しきい値4。送信者には成功を返す（弾かれたと分かると回避を試みるため）。14ケースの判定テスト全通過。**要Supabase SQL実行: `db/2026-07-27-contact-spam.sql`**（未実行でも全て通常受信＝壊れない）。
- **BOOKMARKS室（2026-07-27、実装済み・要Supabase SQL実行）**: 「自分の文章に貼ったリンク=自分のブックマーク」（Andy）。scribe/記事（下書き含む）の本文から外部リンクを走査して`bookmarks`テーブルへ同期（本文がSSOT。リンクが本文から消えたら行も消える）。リンク先タイトルは夜間cron（finalize内、8件/回）+studioの手動ボタン（12件/回）で少しずつ取得（Workers無料のサブリクエスト50/回制限のため一括しない）。`/studio/bookmarks`=検索（タイトル/URL/周辺テキスト）・ドメイン絞り込み・非表示・再スキャン。周辺テキスト（なぜ貼ったか）も保存。自サイト/メディア/中継のURLは除外、iframe埋め込み（Spotifyカード等）は対象外。**要Supabase SQL実行: `db/2026-07-27-bookmarks.sql`**（未実行でも部屋は「SQL未実行」の案内を出すだけで壊れない）。実装時の走査ドライランで既存本文から46リンク検出。
