...
- **検索結果の整備（2026-07-25夜、実装済み）**: description不在でGoogleが本文からページャー矢印ごとスニペットを抜いていた（Andy発見）。全ページにdescription+canonicalを整備: サイト共通=Andyの署名+Aboutリードから組成（`lib/site/about.ts`の`SITE_DESCRIPTION`、CMSのリード変更時は次デプロイで同期）、About=CMSの署名+リード（即反映）、番組=RSSのchannel説明の抜粋（フルフィード。channelInfoは軽量版でdescription空なので使わない）、エピソード=概要欄冒頭、scribe/記事=本文の書き出し（`plainExcerpt`）、棚=機能説明の一行。**文言は全てAndyの既存テキストからの組成**（AI散文をサイトに載せない原則）。JSON-LD: Home=Person+WebSite、番組=PodcastSeries、エピソード=PodcastEpisode（事実データのみ）。canonicalは`?s=1`等のクエリ付きシェアURLを正典に束ねる。**反映待ち**: Googleの再クロールは数日〜数週間。Search Consoleで主要ページのインデックス再登録をリクエストすると早い（Andyの操作）。
...
- **デプロイは自動化済み（2026-07-25夕、Workers Builds）**: mainへの`git push`→Cloudflareが自動ビルド&デプロイ。設定: ビルド`npx opennextjs-cloudflare build`/デプロイ`npx opennextjs-cloudflare deploy`/ルート`/`、非本番ブランチのビルドは無効、ビルド変数=NEXT_PUBLIC_SUPABASE_URL・NEXT_PUBLIC_SUPABASE_ANON_KEY・NEXT_PUBLIC_CF_BEACON_TOKEN（公開値・平文）+SUPABASE_SERVICE_ROLE_KEY（暗号化、ビルド時プリレンダーのDB読みに必須）。GitHub App「Cloudflare Workers and Pages」の権限は**rede-webリポジトリのみ**に限定済み。ビルド状況はダッシュボードのデプロイタブ。**運用は「push一本」**: ローカル`npm run deploy:cf`は自動化が壊れた時の緊急フォールバック限定（併用すると7/23の版ずれ事故の芽になる）。
